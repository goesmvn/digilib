...
export default function ScopusSearchSection({ lang }: { lang: 'id' | 'en' }) {
  ...
  return (
    <section className="py-16 bg-slate-50 text-gray-900 border-t border-b border-slate-200 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-10">
          ...
          <h2 className="text-3xl md:text-4xl font-extrabold tracking-tight text-primary-950 mt-4">
          ...
          <div className="mt-4 p-3 bg-amber-500/10 border border-amber-500/30 rounded-xl text-amber-900 text-xs md:text-sm font-medium inline-flex items-center gap-2 max-w-2xl mx-auto text-left">
        ...
        <form onSubmit={handleSearch} className="max-w-2xl mx-auto mb-12">
          <div className="flex flex-col sm:flex-row gap-3 bg-white p-2 rounded-2xl border border-slate-300 shadow-lg">
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-8 py-3.5 bg-primary-900 hover:bg-primary-800 text-white font-bold rounded-xl text-sm transition-all duration-200 shadow-md shrink-0 flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
        ...
        {/* Error Alert */}
        {error && (
          <div className="max-w-2xl mx-auto mb-8 p-4 bg-red-100 border border-red-300 rounded-xl text-red-700 text-sm font-semibold text-center shadow-sm">
            {error}
          </div>
        )}
        ...
        {/* Results Info & Filter */}
        {total !== null && !error && (
          <div className="mb-6 border-b border-slate-200 pb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-4 flex-wrap">
              <span className="text-sm text-gray-700 font-semibold">{isEn ? `Found ${total} articles` : `Ditemukan ${total} artikel`}</span>
              <button
                onClick={() => setShowSavedModal(true)}
                className="px-3 py-1 bg-amber-500/15 hover:bg-amber-500/25 text-amber-800 border border-amber-500/30 rounded-full text-xs font-bold transition-all flex items-center gap-1.5"
              >
            ...
            <div className="flex items-center gap-2 text-xs">
              <span className="px-2.5 py-1 bg-emerald-100 text-emerald-800 border border-emerald-300 rounded-md font-medium">
                {isEn ? `ScienceDirect: ${results.filter(r => r.isScienceDirect).length}` : `ScienceDirect: ${results.filter(r => r.isScienceDirect).length}`}
              </span>
              <span className="px-2.5 py-1 bg-blue-100 text-blue-800 border border-blue-300 rounded-md font-medium">
              ...
              <span className="text-xs text-primary-900 bg-primary-900/10 px-3 py-1 rounded-full border border-primary-900/20 hidden md:inline-block font-semibold">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {results.map((item) => (
            <div key={item.id} className="bg-white border border-slate-200 hover:border-primary-900/40 rounded-2xl p-6 transition-all duration-300 hover:shadow-xl flex flex-col justify-between group">
              <div>
                <div className="flex items-center justify-between text-xs text-primary-900 font-bold mb-2">
                  <div className="flex items-center gap-2 truncate max-w-[280px]">
                    ...
                    {item.isScienceDirect && (
                      <span className="px-2 py-0.5 bg-emerald-100 text-emerald-800 border border-emerald-300 rounded text-[10px] shrink-0 font-semibold">
                        ScienceDirect
                      </span>
                    )}
                    {item.isOpenAccess && (
                      <span className="px-2 py-0.5 bg-blue-100 text-blue-800 border border-blue-300 rounded text-[10px] shrink-0 font-semibold">
                        Open Access
                      </span>
                    )}
                  </div>
                ...
                <p className="text-xs text-gray-600 font-medium mt-2">
                  <span className="font-semibold text-gray-800">{isEn ? 'Author:' : 'Penulis:'}</span> {item.creator}
                </p>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {item.scopusUrl && (
                    <a
                      ...
                      className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-gray-800 rounded-lg text-xs font-semibold transition-all border border-slate-200"
                    >
                  ...
                  {item.downloadUrl && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="px-3.5 py-1.5 bg-primary-900 hover:bg-primary-800 text-white rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 shadow-sm"
                    >
        ...
        {total !== null && total > pageSize && (
          <div className="mt-10 flex items-center justify-center gap-3">
            <button
              ...
              className="px-4 py-2 bg-white hover:bg-slate-100 border border-slate-300 disabled:opacity-40 disabled:cursor-not-allowed text-gray-700 text-xs font-bold rounded-lg transition-all shadow-sm"
            >
        ...
        {showSavedModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            <div className="bg-white border border-slate-200 rounded-2xl p-6 max-w-2xl w-full text-gray-900 shadow-2xl relative max-h-[80vh] flex flex-col">
              ...
              <div className="overflow-y-auto flex-1 space-y-3 pr-2">
                {savedArticles.length === 0 ? (
                  ...
                  savedArticles.map((article) => (
                    <div key={article.id} className="p-4 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between gap-4">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <h4 className="text-sm font-semibold text-gray-900 truncate">{article.title}</h4>
                          {article.isScienceDirect && (
                            <span className="px-1.5 py-0.5 bg-emerald-100 text-emerald-800 border border-emerald-300 rounded text-[9px] shrink-0 font-semibold">
                              ScienceDirect
                            </span>
                          )}
                          {article.isOpenAccess && !article.isScienceDirect && (
                            <span className="px-1.5 py-0.5 bg-blue-100 text-blue-800 border border-blue-300 rounded text-[9px] shrink-0 font-semibold">
                              Open Access
                            </span>
                          )}
                        </div>
                ...
                <div className="flex items-center gap-2">
                  {savedArticles.length > 0 && (
                    <button
                      ...
                      className="px-3.5 py-2 bg-amber-500 hover:bg-amber-600 text-white text-xs font-bold rounded-lg transition-all flex items-center gap-1.5 shadow-md"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <button
                    onClick={() => setShowSavedModal(false)}
                    className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-xs font-semibold rounded-lg text-gray-700 border border-slate-200"
                  >
                    {isEn ? 'Close' : 'Tutup'}
                  </button>
                </div>
