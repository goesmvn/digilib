...
export default function ScopusSearchSection({ lang }: { lang: 'id' | 'en' }) {
  ...
  return (
    <section className="py-16 bg-[#1e293b] text-slate-100 border-t border-b border-slate-700/60 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        
        <div className="text-center max-w-3xl mx-auto mb-10">
          <span className="px-3.5 py-1.5 bg-amber-500/15 text-amber-300 border border-amber-500/30 rounded-full text-xs font-bold uppercase tracking-wider">
          ...
          <div className="mt-4 p-3.5 bg-slate-900/80 border border-amber-500/30 rounded-xl text-amber-200/90 text-xs md:text-sm font-medium inline-flex items-center gap-2.5 max-w-2xl mx-auto text-left shadow-md">
        ...
        <form onSubmit={handleSearch} className="max-w-2xl mx-auto mb-12">
          <div className="flex flex-col sm:flex-row gap-3 bg-slate-900/90 p-2.5 rounded-2xl border border-slate-700/80 shadow-xl backdrop-blur-md">
            <input
              type="text"
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder={isEn ? 'Search keywords, author, or paper title...' : 'Cari kata kunci, penulis, atau judul artikel...'}
              className="flex-1 bg-transparent px-4 py-3 text-white placeholder-slate-400 outline-none text-sm md:text-base"
            />
            <button
              type="submit"
              disabled={loading}
              className="px-8 py-3.5 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold rounded-xl text-sm transition-all duration-200 shadow-md shrink-0 flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
        ...
        {/* Error Alert */}
        {error && (
          <div className="max-w-2xl mx-auto mb-8 p-4 bg-red-950/60 border border-red-500/40 rounded-xl text-red-200 text-sm font-semibold text-center shadow-sm">
            {error}
          </div>
        )}
        ...
        {/* Results Info & Filter */}
        {total !== null && !error && (
          <div className="mb-6 border-b border-slate-700/60 pb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-4 flex-wrap">
              <span className="text-sm text-slate-300 font-semibold">{isEn ? `Found ${total} articles` : `Ditemukan ${total} artikel`}</span>
              <button
            ...
            <div className="flex items-center gap-2 text-xs">
              <span className="px-2.5 py-1 bg-emerald-950/80 text-emerald-300 border border-emerald-500/30 rounded-md font-medium">
                {isEn ? `ScienceDirect: ${results.filter(r => r.isScienceDirect).length}` : `ScienceDirect: ${results.filter(r => r.isScienceDirect).length}`}
              </span>
              <span className="px-2.5 py-1 bg-blue-950/80 text-blue-300 border border-blue-500/30 rounded-md font-medium">
              ...
              <span className="text-xs text-amber-400 bg-amber-400/10 px-3 py-1 rounded-full border border-amber-400/20 hidden md:inline-block font-semibold">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {results.map((item) => (
            <div key={item.id} className="bg-slate-900/80 border border-slate-700/70 hover:border-amber-500/50 rounded-2xl p-6 transition-all duration-300 hover:shadow-2xl flex flex-col justify-between group">
              <div>
                <div className="flex items-center justify-between text-xs text-amber-400 font-bold mb-2">
                  <div className="flex items-center gap-2 truncate max-w-[280px]">
                    ...
                    {item.isScienceDirect && (
                      <span className="px-2 py-0.5 bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 rounded text-[10px] shrink-0 font-semibold">
                        ScienceDirect
                      </span>
                    )}
                    {item.isOpenAccess && (
                      <span className="px-2 py-0.5 bg-blue-500/20 text-blue-300 border border-blue-500/30 rounded text-[10px] shrink-0 font-semibold">
                        Open Access
                      </span>
                    )}
                  </div>
                  <span className="text-slate-400 font-medium">{item.coverDate?.substring(0, 4) || ''}</span>
                </div>
                ...
                <p className="text-xs text-slate-300 font-medium mt-2">
                  <span className="font-semibold text-slate-400">{isEn ? 'Author:' : 'Penulis:'}</span> {item.creator}
                </p>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {item.downloadUrl && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="px-3.5 py-1.5 bg-amber-500 hover:bg-amber-400 text-slate-950 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 shadow-sm"
                    >
        ...
        {total !== null && total > pageSize && (
          <div className="mt-10 flex items-center justify-center gap-3">
            <button
              ...
              className="px-4 py-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 disabled:opacity-40 disabled:cursor-not-allowed text-slate-200 text-xs font-bold rounded-lg transition-all shadow-sm"
            >
        ...
        {showSavedModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
            <div className="bg-slate-900 border border-slate-700/80 rounded-2xl p-6 max-w-2xl w-full text-slate-100 shadow-2xl relative max-h-[80vh] flex flex-col">
              ...
              <div className="overflow-y-auto flex-1 space-y-3 pr-2">
                {savedArticles.length === 0 ? (
                  ...
                  savedArticles.map((article) => (
                    <div key={article.id} className="p-4 bg-slate-800/80 border border-slate-700 rounded-xl flex items-center justify-between gap-4">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <h4 className="text-sm font-semibold text-white truncate">{article.title}</h4>
                          {article.isScienceDirect && (
                            <span className="px-1.5 py-0.5 bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 rounded text-[9px] shrink-0 font-semibold">
                              ScienceDirect
                            </span>
                          )}
                          {article.isOpenAccess && !article.isScienceDirect && (
                            <span className="px-1.5 py-0.5 bg-blue-500/20 text-blue-300 border border-blue-500/30 rounded text-[9px] shrink-0 font-semibold">
                              Open Access
                            </span>
                          )}
                        </div>
                        {article.doi && <p className="text-xs text-slate-400 font-mono mt-0.5">DOI: {article.doi}</p>}
                        <p className="text-[10px] text-amber-400/80 font-medium mt-1">{isEn ? 'Saved on:' : 'Disimpan:'} {article.savedAt}</p>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="px-3 py-1.5 bg-amber-500 hover:bg-amber-400 text-slate-950 text-xs font-bold rounded-lg transition-all shadow-sm"
                        >
              ...
              <div className="pt-4 border-t border-slate-800 mt-4 flex items-center justify-between gap-3 flex-wrap">
                ...
                <div className="flex items-center gap-2">
                  {savedArticles.length > 0 && (
                    <button
                      ...
                      className="px-3.5 py-2 bg-amber-500 hover:bg-amber-400 text-slate-950 text-xs font-bold rounded-lg transition-all flex items-center gap-1.5 shadow-md"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <button
                    onClick={() => setShowSavedModal(false)}
                    className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-xs font-semibold rounded-lg text-slate-200 border border-slate-700"
                  >
                    {isEn ? 'Close' : 'Tutup'}
                  </button>
                </div>
